...
BASE_INPUT_DIR = r'G:\Carbon_Plate_Shoes_Data\STO_ForDeepLearning\Data_CSV_Split\High_Speed\Interpolte_data\Interpolte_data'
BASE_OUTPUT_DIR = r'G:\Carbon_Plate_Shoes_Data\STO_ForDeepLearning\Data_CSV_Split\High_Speed\Interpolte_data\Interpolte_huizhong_data'
...
# 3. 文件匹配模式与列名 (保持不变)
TARGET_FILE_PATTERN = 'LBSModel-scaled_StaticOptimization_force.csv'
TARGET_COLUMNS = [
...
def save_results(result, output_dir, stiffness):
    ...
    for col, df in result.items():
        if df.empty:
            ...
            continue
        save_path = os.path.join(output_dir, f'AT_{col}.csv')
...
if __name__ == '__main__':
    ...
    for runner_type in RUNNER_TYPES:
        ...
        for stiffness in STIFFNESS_LIST:
            ...
            current_output_dir = os.path.join(BASE_OUTPUT_DIR, runner_type, stiffness, 'AT_Force')
