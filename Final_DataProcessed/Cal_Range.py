...
OUTPUT_DIR = r'G:\Carbon_Plate_Shoes_Data\STO_ForDeepLearning\Data_CSV_Split\High_Speed\uninterpolte\Characteristic_Value\Range_Value'
...
def process_single_feature(filename, file_info_list, output_path):
    """
    处理单个特征文件，计算 Range (Max - Min)，并按宽格式汇总。
    """
    group_dfs_list = []
        ...
        for target_stiffness in STIFFNESSES:
            ...
            if matched_info:
                ...
                try:
                    ...
                    # =======================================================
                    # 🔥 修改点：计算 Range (最大值 - 最小值)
                    # =======================================================
                    # df.max() - df.min() 会自动对每一列进行运算
                    feature_series = df.max() - df.min()
    ...
    if group_dfs_list:
        ...
        # 保存文件，前缀改为 Summary_Range_
        save_file = os.path.join(output_path, f"Summary_Range_{filename}")
        master_df.to_csv(save_file, index=False)
        print(f"  ✅ 已生成 Range 汇总表: {save_file}")
    else:
...
if __name__ == '__main__':
    ...
    print(f"📊 共找到 {len(files_dict)} 种特征文件。开始提取 Range (活动范围)...")
