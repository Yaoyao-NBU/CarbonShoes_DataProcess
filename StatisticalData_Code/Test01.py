...
TARGET_FILE = r'G:\Carbon_Plate_Shoes_Data\Carbon_Fiber_Shoes_DataProcessed\Speed_Spilt\High_Speed\Summary_Statistics_UnInterpolte\MaX_Value/Summary_Max_IK_ankle_angle_r.csv'

# 结果保存的文件夹
OUTPUT_DIR = r'G:\Carbon_Plate_Shoes_Data\Carbon_Fiber_Shoes_DataProcessed\Speed_Spilt\High_Speed\Summary_Statistics_UnInterpolte\MaX_Value/Single_File_Stats_Result'

# 🎯 新增：在这里自定义您的图表 Y 轴标签！(例如：'Peak Ankle Angle (°)', 'Force (BW)')
Y_AXIS_LABEL = 'Peak Ankle Angle (°)'
...
def generate_auto_bar_plot(desc_stats, posthoc_df, feature_name, save_dir, y_label):
    ...
    ax.bar(x - width / 2, amateur_data['Mean'], width, yerr=amateur_data['SD'],
           capsize=5, label='Amateur Runners', color='#FF9999', edgecolor='black',
           linewidth=1.5, error_kw=dict(lw=1.5, capthick=1.5))

    # 🌟 画精英组柱子，颜色为 #99FF99
    ax.bar(x + width / 2, elite_data['Mean'], width, yerr=elite_data['SD'],
           capsize=5, label='Elite Runners', color='#99FF99', edgecolor='black',
           linewidth=1.5, error_kw=dict(lw=1.5, capthick=1.5))
