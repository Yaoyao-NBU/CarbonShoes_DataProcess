"""
Stance力学数据截取 + COP斜率纠正 + 绘图工具 (2号力台)

处理流程:
  1. Peak检测获取stance时间段
  2. 截取STO文件（补充帧归零）
  3. 用中间部分斜率纠正COPx/COPz异常值
  4. 写回截取后的STO文件
  5. 绘制对比图（原始 vs 纠正后）和组合图
"""
import os
import sys
...
output_dir = r'E:\Python_Learn\CarbonShoes_DataProcess\OPenSIm\Data\output\output_plots_slope'
output_cut_dir = r'E:\Python_Learn\CarbonShoes_DataProcess\OPenSIm\Data\output\output_cut_sto_slope'
...
# COP斜率纠正参数
middle_ratio = 0.3               # 中间部分占比（取30%~70%区间）
...
def plot_single_file(file_data, output_dir):
    ...
    configs = [
        ...
        ('COPx', 'Anteroposterior COPx (Slope Corrected)', 'Position (m)'),
        ('COPz', 'Vertical COPz (Slope Corrected)', 'Position (m)')
    ]

    fig, axes = plt.subplots(3, 1, figsize=(12, 10), sharex=True)
    fig.suptitle(f"{file_data['name']} — Force Plate 2 (Slope Correction)",
...
def plot_single_file_compare(original_data, corrected_data, output_dir):
    """为单个文件绘制原始 vs 纠正后对比图（COPx和COPz）"""
    cop_configs = [
    ...
    fig.suptitle(f"{corrected_data['name']} — COP Slope Correction Comparison",
                 fontsize=14, fontweight='bold')

    for idx, (var, title, ylabel) in enumerate(cop_configs):
        ax = axes[idx]
        ax.plot(original_data['time'], original_data[var],
                linewidth=1.0, color='red', alpha=0.7, label='Original')
        ax.plot(corrected_data['time'], corrected_data[var],
                linewidth=1.2, color='blue', alpha=0.9, label='Slope Corrected')
        ax.set_ylabel(ylabel, fontsize=11)
        ax.set_title(title, fontsize=12, loc='left')
        ax.grid(True, linestyle='--', alpha=0.6)
    ...
    path = os.path.join(output_dir, f"{corrected_data['name']}_compare.png")
    plt.savefig(path, dpi=dpi, bbox_inches='tight')
    plt.close()
    return path
...
def plot_trial_overlay(trial_name, files_data, output_dir):
    ...
    configs = [
        ...
        ('COPx', 'Center of Pressure - COPx (Slope Corrected)', 'Position (m)'),
        ('COPz', 'Center of Pressure - COPz (Slope Corrected)', 'Position (m)')
    ]
...
print(f"异常倍数:     {rate_multiplier}x")
print("=" * 60)
...
all_trials_original = {}   # 保存原始数据用于对比图
processed_count = 0
error_count = 0
...
for trial_name, files in trial_data.items():
    ...
    for file_info in files:
        ...
        try:
            ...
            trial_original_data.append(original_dict)
            processed_count += 1
    ...
    all_trials_original[trial_name] = trial_original_data
...
# 单文件对比图（原始 vs 纠正后）
for trial_name, files_data in all_trials_processed.items():
    original_data = all_trials_original.get(trial_name, [])
    for orig, corr in zip(original_data, files_data):
        path = plot_single_file_compare(orig, corr, output_dir)
        print(f"  [OK] {path}")
