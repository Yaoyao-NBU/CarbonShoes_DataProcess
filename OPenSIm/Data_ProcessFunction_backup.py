...
def process_copx_outliers(data_dict, distance_threshold=0.3, t_stance_start=None, t_stance_end=None):
    """
    对stance阶段的COPx数据进行异常值处理：距离中位数超过阈值的点手动归零
    只处理真实stance阶段的数据，padding_frames保持归零状态

    参数:
        data_dict : 包含COPx数据的字典，应包含以下键：
            - 'time': 时间序列
            - 'COPx': COPx位置序列
            - 'COPy': COPy位置序列（可选）
            - 'COPz': COPz位置序列（可选）
    ...
    原理:
        ...
        2. 计算真实stance阶段COPx的中位数作为参考基准
        3. 计算stance阶段每个点到中位数的绝对距离
        4. 将距离超过阈值（0.3米）的点标记为异常值
        5. 将异常值手动归零
        6. padding_frames区域保持归零状态不变

    注意:
        - 只对真实stance阶段的COPx数据进行异常值处理
        - 如果stance阶段COPx值距离中位数超过0.3米，则该点归零
        - padding_frames区域的COPx数据保持归零不变
        - 其他数据（Fy, COPy, COPz）保持不变
    """
    # 创建输出字典的深拷贝
    import copy
    ...
    # Step 1: 计算真实stance阶段COPx的中位数
    copx_median = np.median(copx_stance)

    # Step 2: 计算stance阶段每个点到中位数的绝对距离
    distance_to_median = np.abs(copx_stance - copx_median)

    # Step 3: 识别异常值：距离超过阈值（0.3米）的点（仅在stance阶段内）
    stance_outlier_mask = (distance_to_median > distance_threshold)

    # Step 4: 将异常值归零（仅在stance阶段内）
    # 首先创建完整的outlier_mask（默认为False）
    outlier_mask = np.zeros(len(time_data), dtype=bool)
    # 将stance阶段的outlier_mask设置为识别到的异常值
    outlier_mask[stance_mask] = stance_outlier_mask

    # Step 5: 将异常值手动归零
    result['COPx'][outlier_mask] = 0.0

    # 统计信息
    outlier_count = np.sum(stance_outlier_mask)
    total_stance_count = len(copx_stance)
    outlier_ratio = outlier_count / total_stance_count * 100 if total_stance_count > 0 else 0

    print(f"[OK] COPx异常值处理完成（仅处理真实stance阶段）:")
    print(f"  - 真实stance时间范围: {t_stance_start:.4f}s - {t_stance_end:.4f}s")
    print(f"  - 真实stance数据点数: {total_stance_count}")
    print(f"  - COPx中位数: {copx_median:.6f} m")
    print(f"  - 距离阈值: {distance_threshold:.3f} m")
    print(f"  - 异常值数量: {outlier_count}/{total_stance_count} ({outlier_ratio:.2f}%)")
    print(f"  - 异常值处理方式: 手动归零")
