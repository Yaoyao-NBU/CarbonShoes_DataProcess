...
def spm_integrated_plot(file_list, factor_A_levels, factor_B_levels,
        ...
        for cluster in si_effect.clusters:
            if cluster.P < alpha:
                ...
                ax.text((cluster.endpoints[0] + cluster.endpoints[1]) / 2, bottom,
                        ...
                        fontsize=18, color='#E35B5B', fontweight='bold')
    ...
    ax.set_title(plot_title, fontsize=26, pad=35, fontweight='bold')

    ax.set_xlabel("Stance (%)", fontsize=26, fontweight='bold')
    ax.set_ylabel("Angle(°)", fontsize=26, fontweight='bold')
    ax.set_xlim(0, 100)
    ax.set_ylim(data_ymin, data_ymax)

    leg = ax.legend(loc='upper right', bbox_to_anchor=(1.18, 1.0), frameon=False, fontsize=18)
    ...
    ax.tick_params(axis='both', which='major', width=axis_linewidth, length=6, direction='out', labelsize=26)
...
if __name__ == "__main__":
    ...
    # 2. 待处理指标列表
    target_metrics = [
        "IK_mtp_angle_r", "IK_ankle_angle_r", "IK_knee_angle_r", "IK_hip_adduction_r",
        "IK_hip_flexion_r", "IK_hip_rotation_r", "IK_subtalar_angle_r",
         "IK_lumbar_bending",
        "IK_lumbar_extension", "IK_lumbar_rotation", "IK_pelvis_list",
        "IK_pelvis_rotation", "IK_pelvis_tilt", "IK_pelvis_tx", "IK_pelvis_ty", "IK_pelvis_tz"
    ]
    ...
    for metric in target_metrics:
        ...
        # 3. 动态拼接 6 个组的文件路径
        current_files = [
            os.path.join(base_dir, r"Amateur_Runner\T1\Joint_angle", f"{metric}.csv"),
            os.path.join(base_dir, r"Amateur_Runner\T2\Joint_angle", f"{metric}.csv"),
            os.path.join(base_dir, r"Amateur_Runner\T3\Joint_angle", f"{metric}.csv"),
            os.path.join(base_dir, f"Elite_Runner\T1\Joint_angle", f"{metric}.csv"),
            os.path.join(base_dir, f"Elite_Runner\T2\Joint_angle", f"{metric}.csv"),
            os.path.join(base_dir, f"Elite_Runner\T3\Joint_angle", f"{metric}.csv"),
        ]
