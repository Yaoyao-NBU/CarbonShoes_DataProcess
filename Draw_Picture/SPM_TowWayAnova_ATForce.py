...
def spm_integrated_plot(file_list, factor_A_levels, factor_B_levels,
    ...
    ax.set_ylabel("BW (%)", fontsize=26, fontweight='bold')
    ax.set_xlim(0, 100)
...
if __name__ == "__main__":
    ...
    save_dir = r"G:\Carbon_Plate_Shoes_Data\STO_ForDeepLearning\Data_CSV_Split\High_Speed\Interpolte_data\SPM_Picture\Test_picture\at_force"
    ...
    for metric in target_metrics:
        ...
        # 3. 动态拼接 6 个组的文件路径
        current_files = [
            os.path.join(base_dir, r"Amateur_Runner\T1\AT_Force", f"{metric}.csv"),
            os.path.join(base_dir, r"Amateur_Runner\T2\AT_Force", f"{metric}.csv"),
            os.path.join(base_dir, r"Amateur_Runner\T3\AT_Force", f"{metric}.csv"),
            os.path.join(base_dir, f"Elite_Runner\T1\AT_Force", f"{metric}.csv"),
            os.path.join(base_dir, f"Elite_Runner\T2\AT_Force", f"{metric}.csv"),
            os.path.join(base_dir, f"Elite_Runner\T3\AT_Force", f"{metric}.csv"),
        ]
